	...
	*/
		...
		*/
		function bitcoin_set_tx_fee($amount_in_satoshi=00000000){
			...
			*/
			...
			$amount_in_bitcoins = bcmul($amount_in_satoshi,100000000);
						...
						if($tmp_is_valid_address["isvalid"] == 1){
							...
							if($tmp_command_executed == 1){
								if($tmp_total_received_in_bitcoin >= 0){
									...
									$output["total_received_in_satoshi"] = (int) floor(bcmul($tmp_total_received_in_bitcoin,100000000)); //Convert Bitcoins to satoshi so we can do math with integers.
			...
			//Create a floor limit of zero
				...
				//Bitcoin connection open?
					if($new_btcclient_connection["return_status"] == 1){
						...
						$tmp_command_executed = 0; // (Binary)
						try{
							$tmp_total_balance_in_bitcoin = rtrim(sprintf('%.8F', $new_btcclient_connection["connection"]->getbalance($bitcoin_address_label, $minimum_confirmations)), '0');
							FB::dump($tmp_total_balance_in_bitcoin);
							$tmp_command_executed = 1;
						}catch(Exception $e){
							$tmp_command_executed = 0;
						}
						...
						//Looks like the command was successfully queried, lets continue with execution of the rest of this function
						if($tmp_command_executed == 1){
							if($tmp_total_balance_in_bitcoin >= 0){
								//Looks like a success!
								$output["return_status"] = 1;

								//Return the values....
								$output["balance_in_bitcoin"] = (string) $tmp_total_balance_in_bitcoin;
								$output["balance_in_satoshi"] = (int) floor(bcmul($tmp_total_balance_in_bitcoin,100000000)); //Convert Bitcoins to satoshi so we can do math with integers.
								FB::log($output);
							}else{
								//Failure
								$output["return_status"] = 101;
		...
		**/
		function bitcoin_sendfrom($bitcoin_address_label='', $send_to_bitcoin_address='', $amount_in_satoshi=0, $minimum_confirmations=1){
			...
			$tmp_amount_in_bitcoins = bdk_satoshi_to_bitcoin_display($amount_in_satoshi);

			if ($tmp_amount_in_bitcoins["return_status"] == 1){
				$amount_in_bitcoins = (double) $tmp_amount_in_bitcoins["btc_display"];
		...
		/**
			...
			Purpose
				Query Bitcoin and send Bitcoins from one account within your wallet to another

			Parameter Explanation
				  bitcoin_address_label [string] | (required) | Which "bitcoin addresses" should the Bitcoins be spent from, addresses are identfied by matching "label".
		send_to_bitcoin_address_label  [string]  | (required) | Which address to send/spend the Bitcoins to.
				     amount_in_satoshi [integer] | (required) | The amount of Bitcoins (in satoshi) -- Don't use leading zeros (00050000) instead use (50000).
				  minimum_cofirmations [integer] | (optional) | Only send bitcoins with this many confirmations (or greater).
			...
			Output Explanation
				...
				return_string
					If the return status is (1) then this output will be set to true

				error_rpc_message
					If an error happened at the Bitcoin level then this will contain that error message (Usually set when return status is not 1)
		**/
		function bitcoin_move($bitcoin_address_label='', $send_to_bitcoin_address_label='', $amount_in_satoshi=0, $minimum_confirmations=1){
			global $bdk_integrity_check, $bdk_settings;
			...
			//Bitcoin connection open?
				if($new_btcclient_connection["return_status"] == 1){
					...
					try{
						$tmp_verifymessage_status = $new_btcclient_connection["connection"]->move($bitcoin_address_label, $send_to_bitcoin_address_label, $amount_in_bitcoins, $minimum_confirmations);
						$tmp_command_success = 1;
					}catch(Exception $e){
					...
					if($tmp_command_success == 1){
						...
						$output["return_string"] = $tmp_verifymessage_status;
		...
		*/
			...
			**/
			...
			/**
				...
				Notes
					uses BCMath. Should be ok to rely on as right output.
			...
			**/
			function bdk_satoshi_to_bitcoin_display($satoshi_value, $round_type=0){
				...
				if($proccess_func == 0){
					...
					$tmp_btc_display = (string) bcdiv($satoshi_value,100000000,8);
					$tmp_btc_display = rtrim($tmp_btc_display,"0");

					$output["btc_display"] = $tmp_btc_display;
